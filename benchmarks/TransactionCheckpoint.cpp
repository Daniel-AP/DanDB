...
namespace {
    ...
    constexpr std::size_t CHECKPOINT_TREE_ENTRY_COUNT = 100'000;

    void benchmark_transaction_checkpoint(benchmark::State& state) {

        const auto update_count = static_cast<std::size_t>(state.range(0));
        auto keys_result = make_sequential_keys(CHECKPOINT_TREE_ENTRY_COUNT);
        if(!keys_result.ok()) {
            state.SkipWithError(keys_result.status().message());
            return;
        }

        auto update_keys_result = make_shuffled_keys(CHECKPOINT_TREE_ENTRY_COUNT);
        ...
        state.counters["updates_per_checkpoint"] = static_cast<double>(update_count);

    }

}

BENCHMARK(benchmark_transaction_checkpoint)
