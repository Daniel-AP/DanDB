...
namespace {
    ...
    void benchmark_btree_random_update(benchmark::State& state) {
        ...
        auto update_keys_result = make_shuffled_keys(entry_count);
        ...
        const auto update_commit_status = pager.commit_transaction();
        if(!update_commit_status.ok()) {
            state.SkipWithError(update_commit_status.message());
            return;
        }
        ...
        const auto entries_per_iteration = static_cast<std::int64_t>(update_keys.size());
...
BENCHMARK(benchmark_btree_random_update)
    ->Name("BTree/RandomUpdate")
    ->Arg(1'000)
    ->Arg(10'000)
    ->Arg(100'000)
