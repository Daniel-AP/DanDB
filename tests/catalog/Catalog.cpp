...
TEST_CASE("Catalog discards a staged secondary index after the caller rolls back", "[catalog][create-index][transaction]") {
    const TempDir temp_dir;
    ...
    REQUIRE(catalog.create_table("users", make_schema_with_indexable_columns()).ok());

    const auto* table = catalog.find_table("users");
    REQUIRE(table != nullptr);
    const auto table_id = table->table_id();

    const auto* age_column = catalog.find_column(table_id, "age");
    REQUIRE(age_column != nullptr);
    const auto age_column_id = age_column->column_id();
    ...
    REQUIRE(catalog.on_transaction_rolled_back().ok());
    REQUIRE(catalog.find_index("users_by_age") == nullptr);
    REQUIRE(catalog.indexes_for_table(table_id).size() == 1);
    ...
    auto reopened_catalog_result = Catalog::load(reopened_pager);
    REQUIRE(reopened_catalog_result.ok());
    REQUIRE(reopened_catalog_result.value().find_index("users_by_age") == nullptr);

    REQUIRE(reopened_pager.close().ok());
}

TEST_CASE("Catalog restores a staged index drop after the caller rolls back", "[catalog][drop-index][transaction]") {
    const TempDir temp_dir;
    ...
    REQUIRE(catalog.create_index(table_id, "users_by_age", age_column->column_id(), false).ok());
...
TEST_CASE("Catalog restores a staged table drop after the caller rolls back", "[catalog][drop-table][transaction]") {
    const TempDir temp_dir;
    ...
    const auto* email_column = catalog.find_column(table_id, "email");
    REQUIRE(email_column != nullptr);
    const auto unique_index_name = dandb::catalog::internal_unique_index_name(
        table_id,
        email_column->column_id()
    );
    ...
    REQUIRE(catalog.find_table("users") == nullptr);
    REQUIRE(catalog.find_index(unique_index_name) == nullptr);
    ...
    REQUIRE(reopened_catalog_result.ok());
    REQUIRE(reopened_catalog_result.value().find_table("users") != nullptr);
    REQUIRE(reopened_catalog_result.value().find_index(unique_index_name) != nullptr);
