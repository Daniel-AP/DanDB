...
namespace dandb::btree {
    ...
    core::Result<std::optional<SplitResult>> BTree::insert_into_leaf(storage::PageId page_id, std::span<std::byte> key, std::span<const std::byte> value) {

    }

    core::Result<std::optional<SplitResult>> BTree::insert_into_internal(storage::PageId page_id, std::span<std::byte> key, std::span<const std::byte> value) {

    }

    storage::PageId BTree::next_child_page_id_to_key(const BTreeInternalPage<const std::byte> page, std::span<std::byte> key) {
